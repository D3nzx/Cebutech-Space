...
function DeanManagement() {
  ...
  const loadDeans = useCallback(async () => {
    setLoading(true);
    try {
      const { data, error: loadError } = await getDeans();
      if (loadError) {
        setError(loadError);
      ...
      setError(err?.message || "An error occurred while loading deans");
      setShowErrorModal(true);
    } finally {
      setLoading(false);
    }
  ...
  const filteredDeans = useMemo(() => {
    ...
    return deans.filter((d) => {
      const code = (d.dean_code ?? "").toLowerCase();
  ...
  const totalPages = Math.max(1, Math.ceil(filteredDeans.length / pageSize));

  const pagedDeans = useMemo(() => {
    const start = (currentPage - 1) * pageSize;
    return filteredDeans.slice(start, start + pageSize);
  }, [filteredDeans, currentPage, pageSize]);
  ...
  const handleToggleStatus = async (row) => {
    ...
    try {
      ...
      const apiCall = newStatus ? enableDeanAccount : disableDeanAccount;
      ...
      if (toggleError) {
        ...
        setDeans((prev) => prev.map((d) => (d.id === row.id ? { ...d, is_active: newStatus } : d)));
  ...
  const confirmDelete = async () => {
    ...
    try {
      const { error: deleteError } = await deleteDean(modalData.id);

      if (deleteError) {
        setError(deleteError);
        setShowErrorModal(true);
      } else {
        setDeans((prev) => prev.filter((d) => d.id !== modalData.id));
        setSuccessMessage("Dean deleted successfully!");
        setShowSuccessModal(true);
      }
    } catch (err) {
      setError(err?.message || "An error occurred while deleting the dean");
      setShowErrorModal(true);
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const { exists, error: existsError } = await checkDeanExists(formData.email.trim(), editingId);
      if (existsError) {
      ...
      if (exists) {
        setError("A dean with this email already exists.");
        setShowErrorModal(true);
        return;
      }
      ...
      const { data, error: saveError } = await updateDean(editingId, payload);
      ...
      if (saveError) {
        ...
        if (data) {
          setDeans((prev) => prev.map((d) => (d.id === editingId ? data : d)));
        } else {
          await loadDeans();
        }
        setSuccessMessage("Dean updated successfully!");
        setShowSuccessModal(true);
  ...
  return (
    <div className="space-y-3">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Dean Management</h2>
              <p className="text-xs text-slate-500 mt-0.5">
                {filteredDeans.length} dean{filteredDeans.length !== 1 ? "s" : ""} found
              </p>
            </div>
            <div className="flex items-center gap-3">
              <div className="relative flex-1 md:flex-none">
                ...
                <input
                  ...
                  placeholder="Find by name, email, or Dean ID..."
          ...
          <table className="w-full border-collapse">
            <thead className="bg-gradient-to-r from-slate-100 to-slate-50 border-b-2 border-slate-300">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider w-32">Dean ID</th>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className="text-slate-600 text-sm">Loading deans...</span>
                    </div>
                  </td>
                </tr>
              )}
              {!loading && pagedDeans.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                    <p className="text-sm font-medium">No deans found. {searchQuery && "Try adjusting your search."}</p>
                  </td>
                </tr>
              )}
              {!loading &&
                pagedDeans.map((row, idx) => (
                  <tr key={row.id} className="hover:bg-blue-50/30 transition-colors duration-150 border-b border-slate-100">
                    ...
                    <td className="px-6 py-4 text-sm font-semibold text-slate-900">{row.dean_code || "—"}</td>
        ...
        <div className="px-4 py-3 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-slate-600">
            Showing <span className="font-semibold">{Math.min((currentPage - 1) * pageSize + 1, filteredDeans.length)}</span> to{" "}
            <span className="font-semibold">{Math.min(currentPage * pageSize, filteredDeans.length)}</span> of{" "}
            <span className="font-semibold">{filteredDeans.length}</span> entries
          </p>
              ...
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-8 py-6 relative">
                ...
                <div className="relative flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0 pr-8">
                    <h2 className="text-2xl font-bold text-white">Edit Dean</h2>
                    <p className="text-blue-100/90 mt-1 text-xs font-medium">Update dean information</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-5">
                      <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6 border-2 border-purple-200 space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-sm font-bold text-slate-900 mb-2">Dean ID</label>
                            <input
                              type="text"
                              value={formData.deanCode}
          ...
          <div className="relative w-full max-w-sm transform rounded-2xl bg-white shadow-2xl transition-all animate-in scale-in-95 fade-in duration-300">
            <div className="bg-gradient-to-r from-red-600 to-red-700 px-6 py-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-white">Remove Dean</h2>
            ...
            <div className="px-6 py-6 space-y-4">
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-2">
                <p className="text-xs font-bold text-slate-700 uppercase tracking-wide">Dean Details</p>
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-slate-600">
                    Dean ID: <span className="font-semibold text-slate-700">{modalData?.dean_code || "—"}</span>
                  </p>
                </div>
              </div>
            </div>
      ...
      <SuccessModal
        ...
        action="The dean list has been updated."
      />
    </div>
  );
}
