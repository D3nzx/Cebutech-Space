...
function StudentHeader({ onMenuClick, user, studentData, onAccountSettings = () => {}, onLogout = () => {}, isSidebarCollapsed = false, isSidebarLocked = false, onToggleSidebar = () => {} }) {
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  const [gender, setGender] = useState(studentData?.gender || null);
  const dropdownRef = useRef(null);

  const { firstName, lastName, fullName, initials } = React.useMemo(() => {
    const first = studentData?.first_name || '';
    const last = studentData?.last_name || '';
  ...
  useEffect(() => {
    const handleStudentProfileUpdated = (event) => {
    ...
    window.addEventListener('studentProfileUpdated', handleStudentProfileUpdated);
    window.addEventListener('studentDataUpdated', handleDataUpdate);
    return () => {
      window.removeEventListener('studentProfileUpdated', handleStudentProfileUpdated);
