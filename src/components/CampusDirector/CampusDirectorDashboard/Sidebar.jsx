...
function Sidebar({ isOpen, onClose, isCollapsed = false, activeSection, onSectionChange, isMobile = false }) {
  const navItems = [
    { name: "Dashboard", icon: LayoutDashboard, section: "dashboard" },
    { name: "Schedule Reports", icon: FileText, section: "reports" },
  ];
    ...
    <div className="flex flex-col h-full">
      <div className={`border-b border-slate-800/50 transition-all duration-300 ${(isCollapsed && !isOpen) ? "p-4" : "p-6"}`}>
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-orange-600 via-red-500 to-red-700 flex items-center justify-center shadow-lg shadow-red-600/30 flex-shrink-0">
            <GraduationCap size={26} className="text-white" />
          </div>
          {(!isCollapsed || isOpen) && (
            <div className="overflow-hidden">
              <h1 className="text-base font-bold tracking-tight whitespace-nowrap text-white">CTU Portal</h1>
            </div>
          )}
        </div>
      </div>
