...
function DeanNotificationsPanel({ deanId, onOpenApproval }) {
  ...
  const loadNotifications = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await getNotifications(deanId, 'dean');
      setNotifications(data || []);
      const { count } = await getUnreadNotificationsCount(deanId, 'dean');
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`dean_notifications_${deanId}`)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        table: 'notifications',
        filter: `recipient_id=eq.${deanId} and recipient_type=eq.dean`
      }, (payload) => {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      await markAllNotificationsAsRead(deanId, 'dean');
  ...
  const handleDecision = async (isApprove, comment) => {
    if (!activeRequest || !deanId) return;
    ...
    try {
      ...
      // Notify Campus Director if approved
      if (isApprove) {
...
export default DeanNotificationsPanel;
