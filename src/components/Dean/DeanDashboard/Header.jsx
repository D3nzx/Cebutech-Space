...
import DeanNotificationsPanel from "./DeanNotificationsPanel";
...
function Header({
  ...
  const { fullName, initials } = React.useMemo(() => {
    const first = deanData?.first_name || user?.user_metadata?.firstName || user?.user_metadata?.first_name || '';
    const last = deanData?.last_name || user?.user_metadata?.lastName || user?.user_metadata?.last_name || '';
  ...
  useEffect(() => {
    if (deanData && deanData.gender !== undefined) {
      setGender(deanData.gender);
    }
  }, [deanData]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('deanProfileUpdated', handleProfileUpdate);
    window.addEventListener('deanDataUpdated', handleDataUpdate);
    return () => {
      window.removeEventListener('deanProfileUpdated', handleProfileUpdate);
  ...
  return (
    <div className="flex items-center justify-between px-6 py-4">
      ...
      <div className="flex items-center gap-3" ref={dropdownRef}>
        <DeanNotificationsPanel deanId={deanData?.id} />
