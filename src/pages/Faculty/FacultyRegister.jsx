...
import { registerFaculty } from '../../api/facultyAuth';
...
function FacultyRegister() {
  ...
  useEffect(() => {
    const savedData = getSavedFormData('facultyRegisterForm');
    if (savedData) {
      console.log('♻️ Restoring Faculty Register form data on mount');
  ...
  const formDataState = {
    ...
    collegeId, college, programId, program, currentStep
  };
  useFormPersistence('facultyRegisterForm', formDataState);
  useClearFormOnUnmount('facultyRegisterForm');

 
  useEffect(() => {
    document.title = 'CTU Portal | Faculty Registration';
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      sessionStorage.setItem('registrationUserType', 'faculty');
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-slate-950 to-black flex items-center justify-center px-3 sm:px-4 py-4 sm:py-2 overflow-hidden">
      <div className="w-full max-w-2xl lg:max-w-3xl bg-white/5 border border-white/10 rounded-2xl sm:rounded-3xl shadow-2xl backdrop-blur-xl overflow-hidden outline-none focus:outline-none">
        <div className="grid lg:grid-cols-2 auto-rows-max outline-none focus:outline-none">
          <div className="p-5 sm:p-6 lg:p-7 text-white bg-gradient-to-br from-red-600 via-red-500 to-orange-500 pointer-events-none select-none" aria-hidden="true">
          </div>
          ...
          <div className="p-4 sm:p-5 md:p-5 lg:p-6 bg-white/95 flex items-stretch select-none">
            <div className="w-full max-w-full sm:max-w-md mx-auto">
              <div className="flex flex-col items-center text-center mb-3 sm:mb-4 pointer-events-none">
                ...
                <h2 className="text-base sm:text-lg font-semibold text-slate-900">Faculty Registration</h2>
          ...
          <div className="bg-white/95 border border-white/10 rounded-3xl shadow-2xl p-8 w-full max-w-sm backdrop-blur-xl">
            ...
            <div className="bg-[#4285F4]/10 border border-[#4285F4]/30 rounded-2xl p-4 mb-6">
              <p className="text-sm text-slate-700 leading-relaxed">
                <span className="font-semibold">Please check your email</span> (including spam/junk folders) and click the confirmation link to verify your account. This step is essential to activate your Faculty account and gain full access to the system.
              </p>
