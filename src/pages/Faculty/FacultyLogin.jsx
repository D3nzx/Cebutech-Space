...
import { loginFaculty } from '../../api/facultyAuth';
...
function FacultyLogin() {
  ...
  useEffect(() => {
    const savedData = getSavedFormData('facultyLoginForm');
    if (savedData) {
      console.log('♻️ Restoring Faculty Login form data on mount');
  ...
  useFormPersistence('facultyLoginForm', formDataState);
  useClearFormOnUnmount('facultyLoginForm');

  useEffect(() => {
    document.title = 'CTU Portal | Faculty Login';
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed?.target === 'faculty' && parsed?.message) {
  ...
  const handleSendReset = async () => {
    ...
    const redirectTo = `${window.location.origin}/faculty/login`;
    ...
    try {
      ...
      if (error) {
        ...
        setTimeout(async () => {
          ...
          window.history.replaceState(null, '', '/faculty/login');
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-slate-950 to-black flex items-center justify-center px-4 py-6">
      <div className="w-full max-w-2xl bg-white/5 border border-white/10 rounded-3xl shadow-2xl backdrop-blur-xl overflow-hidden outline-none focus:outline-none">
        <div className="grid md:grid-cols-2 outline-none focus:outline-none">
          <div className="p-8 md:p-8 text-white bg-gradient-to-br from-red-600 via-red-500 to-orange-500 pointer-events-none select-none" aria-hidden="true">
          </div>
