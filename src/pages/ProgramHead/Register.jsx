...
import { signUp, checkProgramHeadExists } from '../../api/auth';
...
function Register() {
  ...
  useEffect(() => {
    const savedData = getSavedFormData('programHeadRegisterForm');
    if (savedData) {
      console.log('♻️ Restoring Program Head Register form data on mount');
  ...
  useFormPersistence('programHeadRegisterForm', formDataState);
  useClearFormOnUnmount('programHeadRegisterForm');
  ...
  useEffect(() => {
    document.title = 'CTU Portal | Registration';
  }, []);
  ...
  const [errors, setErrors] = useState({ firstName: '', lastName: '', email: '', password: '', confirmPassword: '', college: '', program: '', general: '' });
  ...
  const [programHeadExists, setProgramHeadExists] = useState(false);
  const [checkingProgramHead, setCheckingProgramHead] = useState(false);
  const [showProgramHeadAssignedModal, setShowProgramHeadAssignedModal] = useState(false);
  const [assignedProgramLabel, setAssignedProgramLabel] = useState('');
  ...
  useEffect(() => {
    const checkProgramHeadAssignment = async () => {
      if (!program || !college) {
        setProgramHeadExists(false);
        setAssignedProgramLabel('');
        return;
      }

      setCheckingProgramHead(true);
      try {
        ...
        if (selectedProgram && selectedCollege) {
          const { exists } = await checkProgramHeadExists(selectedCollege.college_name, selectedProgram.course_name);
          setProgramHeadExists(exists);
          if (exists) {
            console.warn(`⚠️ Program Head already assigned to ${selectedCollege.college_name} - ${selectedProgram.course_name}`);
            setAssignedProgramLabel(selectedProgram.course_name);
            setShowProgramHeadAssignedModal(true);
          } else {
            setAssignedProgramLabel('');
          }
        }
      } catch (err) {
        console.error('Error checking program head assignment:', err);
        setProgramHeadExists(false);
        setAssignedProgramLabel('');
      } finally {
        setCheckingProgramHead(false);
      }
    };

    checkProgramHeadAssignment();
  }, [program, college, programs, colleges]);
  ...
  const validate = (scope = 'all') => {
    let valid = true;
    const newErrors = { firstName: '', lastName: '', email: '', password: '', confirmPassword: '', college: '', program: '', general: '' };
    const checkStep1 = scope === 'all' || scope === 1;
    const checkStep2 = scope === 'all' || scope === 2;
  ...
  const handleNextStep = () => {
    const { valid, newErrors } = validate(1);
    if (!valid) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSuccess('');
    setError('');
    const { valid, newErrors } = validate('all');
    ...
    sessionStorage.setItem('registrationUserType', 'program_head');
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-slate-950 to-black flex items-center justify-center px-3 sm:px-4 py-4 sm:py-2 overflow-hidden">
      <div className="w-full max-w-2xl lg:max-w-3xl bg-white/5 border border-white/10 rounded-2xl sm:rounded-3xl shadow-2xl backdrop-blur-xl overflow-hidden">
        <div className="grid lg:grid-cols-2 auto-rows-max">
          <div className="p-5 sm:p-6 lg:p-7 text-white bg-gradient-to-br from-red-900/60 via-red-700/70 to-red-600/70">
          </div>
          ...
          <div className="p-4 sm:p-5 md:p-5 lg:p-6 bg-white/95 flex items-stretch">
            <div className="w-full max-w-full sm:max-w-md mx-auto">
              ...
              <h2 className="text-base sm:text-lg font-semibold text-slate-900">Department Chair Registration</h2>
                  ...
                  <div key={step} className="flex items-center gap-1.5">
                    <div
                      className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold ${
                        currentStep === step ? 'bg-blue-500 text-white' : 'bg-slate-200 text-slate-600'
                      }`}
                    >
                      {step}
                    </div>
                    <span className={`text-xs font-semibold ${currentStep === step ? 'text-slate-900' : 'text-slate-500'}`}>
              ...
              {currentStep === 2 && (
                <>
                  ...
                  <div className="pt-0.5">
                    <p className="text-[9px] sm:text-[10px] font-semibold uppercase tracking-[0.1em] sm:tracking-[0.12em] text-slate-400 mb-1.5">
                      Academic Details
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 sm:space-y-2.5">
                    ...
                    <div>
                      ...
                      <ProgramSelector
                        ...
                        colleges={colleges || []}
                        value={program}
                        onChange={(selectedProgramId) => setProgram(selectedProgramId)}
                  ...
                  <div className="flex flex-col sm:flex-row gap-1.5 justify-between pt-1.5">
                    <button
                      type="button"
                      disabled={isLoading}
                    ...
                    <button
                      type="submit"
                      disabled={isLoading || programHeadExists || checkingProgramHead}
                      ...
                      title={programHeadExists ? 'Registration unavailable - Program Head already assigned' : ''}
                    >
                      ...
                      {checkingProgramHead ? 'Checking...' : isLoading ? 'Creating…' : 'Create Account'}
                    </button>
                  </div>
                </>
              )}
      ...
      {showProgramHeadAssignedModal && programHeadExists && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white/95 border border-white/10 rounded-3xl shadow-2xl p-8 w-full max-w-sm backdrop-blur-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-bold text-slate-900">Program Head Already Assigned</h2>
            </div>

            <p className="text-sm text-slate-600 mb-4 leading-relaxed">
              A Program Head has already been assigned to{assignedProgramLabel ? ` ${assignedProgramLabel}` : ' this program'}. Only one Program Head can be registered per program.
            </p>

            <div className="space-y-3">
              <button
                type="button"
                onClick={() => setShowProgramHeadAssignedModal(false)}
      ...
      {showEmailVerificationModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div
            className="bg-white/95 border border-white/10 rounded-3xl shadow-2xl p-8 w-full max-w-sm backdrop-blur-xl"
          >
            ...
            <div className="bg-[#4285F4]/10 border border-[#4285F4]/30 rounded-2xl p-4 mb-6">
              <p className="text-sm text-slate-700 leading-relaxed">
                <span className="font-semibold">Please check your email</span> (including spam/junk folders) and click the confirmation link to verify your account. This step is essential to activate your Program Head account and gain full access to the system.
              </p>
