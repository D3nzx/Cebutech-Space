  ...
  useEffect(() => {
    const handleAuthCallback = async () => {
      try {
        ...
        console.log('✅ Email confirmed! Database trigger will create profile automatically.');

        
        if (user.email_confirmed_at) {
          console.log('✅ Email confirmed! Showing verification success message');
          sessionStorage.setItem('registrationSuccessEmail', user.email);
          setStatus('success');
          
          setTimeout(() => {
            navigate(
              `${userType === 'dean' ? '/dean/registration-success' : '/programhead/registration-success'}?email=${encodeURIComponent(user.email)}`,
              { replace: true }
            );
          }, 3000);
        } else {
