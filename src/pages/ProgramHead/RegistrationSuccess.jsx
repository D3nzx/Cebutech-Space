...
function RegistrationSuccess() {
  ...
  useEffect(() => {
    ...
    const typeParam = searchParams.get('type') || sessionStorage.getItem('registrationUserType') || 'program_head';
    ...
    sessionStorage.setItem('registrationUserType', typeParam);
    ...
    switch(userType) {
      case 'faculty':
        return {
          ...
          dashboardAccess: 'You can now access the dashboard to view your teaching schedule, manage courses, and access academic resources.',
          role: 'Faculty Member'
        };
      case 'student':
        return {
          profileType: 'Student profile',
          dashboardAccess: 'You can now access the portal to view your enrolled courses, check class schedules, and access academic information.',
          role: 'Student'
        };
      case 'program_head':
      default:
        return {
          profileType: 'Program Head profile',
          dashboardAccess: 'You can now access the dashboard to manage courses, faculty, and schedules.',
          role: 'Program Head'
        };
    }
  };
        ...
        <div className="bg-white/95 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="px-5 py-6 md:px-6 md:py-7">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 md:p-4 text-center">
              ...
              <p className="text-slate-500 text-xs mt-1">
                An administrator is currently reviewing the information you submitted. Please wait for approval, or contact the administrator for updates.
              </p>
            </div>
          </div>
          ...
          <div className="bg-slate-50 px-5 py-3 md:px-6 border-t border-slate-200">
            <p className="text-center text-slate-600 text-xs">
              Having trouble? <a href="mailto:support@ctu.edu.ph" className="text-blue-600 hover:text-blue-700 font-semibold transition">Contact Support</a>
            </p>
          </div>
        </div>

        {/* Security Notice - Optional */}
        <div className="mt-4 px-4 py-2 bg-white/5 border border-white/10 rounded-xl backdrop-blur-sm">
          <p className="text-slate-300 text-xs text-center">
            <span className="text-slate-400">🔒</span> Your account is secure. Never share your login credentials.
          </p>
        </div>
      </div>
    </div>
