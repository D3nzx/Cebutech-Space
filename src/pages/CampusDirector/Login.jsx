...
import { loginCampusDirector } from '../../api/auth';
...
function Login() {
  ...
  useEffect(() => {
    const savedData = getSavedFormData('campusDirectorLoginForm');
    if (savedData) {
  ...
  useFormPersistence('campusDirectorLoginForm', formDataState);
  useClearFormOnUnmount('campusDirectorLoginForm');

  useEffect(() => {
    document.title = 'CTU Portal | Campus Director Login';
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed?.target === 'campus_director' && parsed?.message) {
  ...
  const handleSendReset = async () => {
    ...
    const redirectTo = `${window.location.origin}/campusdirector/login`;
  ...
  useEffect(() => {
    ...
    const isRecovery = hash.includes('type=recovery') || hash.includes('access_token') || searchParams.get('type') === 'recovery';
    ...
    try {
      ...
      if (error) {
        ...
        setTimeout(async () => {
          ...
          window.history.replaceState(null, '', '/campusdirector/login');
          window.location.href = '/campusdirector/login';
        }, 1500);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await loginCampusDirector({ email, password });

    if (result.success) {
      setSuccessMsg('Login successful! Accessing your dashboard...');
      setShowSuccessModal(true);
      setTimeout(() => {
        navigate('/campusdirector/dashboard', { replace: true });
      }, 2000);
    } else {
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-slate-950 to-black flex items-center justify-center px-4 py-6">
      <div className="w-full max-w-2xl bg-white/5 border border-white/10 rounded-3xl shadow-2xl backdrop-blur-xl overflow-hidden">
        <div className="grid md:grid-cols-2">
          <div className="p-8 md:p-8 text-white bg-gradient-to-br from-orange-600 via-red-500 to-red-700"></div>

          <div className="p-8 md:p-8 bg-white/95">
            <div className="flex flex-col items-center text-center mb-6">
              <img src={CTULogo} alt="CTU Logo" className="w-14 h-14 mb-3" />
              <h2 className="text-xl font-semibold text-slate-900">Campus Director Login</h2>
              <p className="text-sm text-slate-500">Sign in to continue</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4">
              {securityNotice && (
                <div className="rounded-2xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-xs text-indigo-900 flex items-start gap-3">
              ...
              <div className="text-center pt-2">
                <p className="text-xs md:text-sm text-slate-600">
                  ...
                  <button
                    ...
                    tabIndex={0}
                    onClick={() => navigate('/campusdirector/register')}
              ...
              {recoveryMsg && (
                <div
                  className={`text-xs p-3 rounded-xl ${
                    recoveryMsg.includes('successfully') || recoveryMsg.includes('Redirecting')
                      ? 'bg-green-50 text-green-700 border border-green-200'
                      : 'bg-red-50 text-red-700 border border-red-200'
                  }`}
                >
