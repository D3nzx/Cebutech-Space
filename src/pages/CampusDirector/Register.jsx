...
import { signUpCampusDirector, checkCampusDirectorExists } from '../../api/auth';
...
function Register() {
  ...
  const [campusDirectorExists, setCampusDirectorExists] = useState(false);
  const [checkingCampusDirector, setCheckingCampusDirector] = useState(true);
  ...
  useEffect(() => {
    const savedData = getSavedFormData('campusDirectorRegisterForm');
    if (savedData) {
  ...
  useFormPersistence('campusDirectorRegisterForm', formDataState);
  useClearFormOnUnmount('campusDirectorRegisterForm');
  ...
  useEffect(() => {
    document.title = 'CTU Portal | Campus Director Registration';
  }, []);
  ...
  useEffect(() => {
    const run = async () => {
      setCheckingCampusDirector(true);
      const { exists } = await checkCampusDirectorExists();
      setCampusDirectorExists(Boolean(exists));
      setCheckingCampusDirector(false);
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (checkingCampusDirector || campusDirectorExists) {
      setErrorModalTitle('Campus Director Account Already Exists');
      setErrorModalMessage(
        'A Campus Director account has already been created. Only one Campus Director can be registered in the system. If you believe this is an error, please contact the administrator.'
      );
    ...
    const { user, error: signUpError } = await signUpCampusDirector({ email, password, firstName, lastName });
    ...
    sessionStorage.setItem('registrationUserType', 'campus_director');
          ...
          <div className="p-4 sm:p-5 md:p-5 lg:p-6 bg-white/95 flex items-stretch">
            <div className="w-full max-w-full sm:max-w-md mx-auto">
              <div className="flex flex-col items-center text-center mb-3 sm:mb-4">
                ...
                <h2 className="text-base sm:text-lg font-semibold text-slate-900">Campus Director Registration</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-2.5">
                ...
                <div className="flex justify-center pt-1.5">
                  <button
                    type="submit"
                    disabled={isLoading || campusDirectorExists || checkingCampusDirector}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white/95 border border-white/10 rounded-3xl shadow-2xl p-8 w-full max-w-sm backdrop-blur-xl">
            ...
            <div className="space-y-3">
              <button
                onClick={() => navigate('/campusdirector/login', { replace: true })}
